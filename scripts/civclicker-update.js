...
function addRaidRows()
{
	...
	civSizes.forEach(function(elem) { 
		s += "<div class='col-3'><button class='raid w-100' data-action='raid' data-target='"+elem.id+"' disabled='disabled'>"+
		elem.name+"</button></div>"; //xxxL10N
	});
    s += '</div>'
...
function updateMorale(){
	...
	elt.className = "row gx-1 happy-" + happinessRank;
}

function addWonderSelectText() {
    
	var wcElem = ui.find("#wonderCompletedBonuses");
	...
	wonderResources.forEach(function(elem,i, wr) {
		s += "<div class='col-2'><button class='w-100 text-capitalize' onmousedown='wonderSelect(\"" +elem.id+"\")'>"+elem.getQtyName(0)+"</button></div>";
	});
